...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.folder_edit = QLineEdit(str(Path.cwd()))
        ...
        # Splitter: upper (options + preview) / lower (output)
        splitter = QSplitter()
        ...
        self.token_preview_label = QLabel("Token preview:")
        ...
        self.output_editor.setReadOnly(False)
        ...
        self.folder_edit.textChanged.connect(self._update_token_preview)
        for cb in (
            self.compress_cb,
    ...
    def _on_preset_changed(self, index: int) -> None:
        ...
        if not preset:
            return
        config = self._build_config(preview=False)
    ...
    def _on_browse(self) -> None:
        current = Path(self.folder_edit.text() or ".").expanduser().resolve()
    ...
    def _update_token_preview(self) -> None:
        config = self._build_config(preview=True)
        try:
            xml_text, stats = generate_xml(config, use_tqdm=False, preview=True)
